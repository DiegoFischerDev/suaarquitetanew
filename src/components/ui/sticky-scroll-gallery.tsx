...
function GalleryIntro({
  ...
  const innerClass = sticky
    ? "sticky top-0 z-10 grid h-screen w-full place-content-center bg-cream"
    : "mx-auto max-w-2xl";
    ...
    <div className={wrapperClass}>
      <div className={innerClass}>
        ...
        <div className={`relative ${sticky ? "px-6 text-center" : ""}`}>
          <h3 className="heading-display text-3xl font-medium leading-tight text-ink md:text-5xl">
            {introTitle}
          </h3>
      ...
      <div className="mx-auto grid max-w-[90rem] grid-cols-12 gap-3">
        <div className="col-span-4 grid gap-3">
          {columns.left.map((image) => (
            <GalleryFigure key={image.src} image={image} className="h-[28rem]" />
          ))}
        </div>
        ...
        >
          {columns.center.map((image) => (
            <GalleryFigure key={image.src} image={image} className="h-full min-h-0" />
          ))}
        </div>
    ...
    <div className="w-full bg-cream">
      {showDesktop ? (
        ...
        <>
          ...
          <ImageAutoSlider images={images} className="pb-12" />
        </>
      )}
    </div>
  );
