...
function ShaderCanvas({
  ...
  useEffect(() => {
    ...
    if (!vs) {
      console.error(`Vertex compile error: ${vsLog}`);
      return cleanup;
    }
    ...
    if (!fs) {
      console.error(`Fragment compile error: ${fsLog}`);
      gl.deleteShader(vs);
      return cleanup;
    }
    ...
    gl.deleteShader(fs);
    if (!linked.program) {
      console.error(`Program link error: ${linked.log}`);
      return cleanup;
    }
...
function StaticFallback({ className = "" }: { className?: string }) {
  return (
    <div
      className={`bg-gradient-to-br from-[#050a18] via-[#0a1530] to-[#081228] ${className}`}
      aria-hidden="true"
    />
  );
}

type ShaderGridBackgroundProps = {
  className?: string;
  children?: ReactNode;
};
...
export function ShaderGridBackground({
  className = "",
  children,
}: ShaderGridBackgroundProps) {
  const reduceMotion = useReducedMotion();

  return (
    <div className={`absolute inset-0 overflow-hidden ${className}`}>
      {reduceMotion ? (
        <StaticFallback className="h-full w-full" />
      ) : (
